...
entity WorkItems : cuid, managed {
  ...
  workItemType     : String(40) @title: 'Type';               // Project/Enhancement/Break-fix/Support/Hypercare/Upgrade/Compliance Initiative/etc.
  ...
  risks             : Composition of many Risks on risks.workItem = $self;
  actionItems       : Composition of many ActionItems on actionItems.workItem = $self;
  progressSnapshots : Composition of many ProgressSnapshots on progressSnapshots.workItem = $self;
}
...
// ─── Risk Register (per work item) ───
entity Risks : cuid, managed {
  workItem    : Association to WorkItems;
  description : String(500) @title: 'Risk Description';
  likelihood  : String(10) default 'Medium' @title: 'Likelihood';  // Low/Medium/High
  impact      : String(10) default 'Medium' @title: 'Impact';      // Low/Medium/High
  riskScore   : Integer default 0 @title: 'Risk Score';            // auto: likelihood×impact (1-9)
  owner       : String(120) @title: 'Risk Owner';
  mitigation  : String(500) @title: 'Mitigation Plan';
  status      : String(20) default 'Open' @title: 'Status';        // Open/Mitigated/Accepted/Closed
  dueDate     : Date @title: 'Target Mitigation Date';
}

// ─── Action Items (per work item parking lot) ───
entity ActionItems : cuid, managed {
  workItem    : Association to WorkItems;
  description : String(500) @title: 'Action Description';
  owner       : String(120) @title: 'Owner';
  dueDate     : Date @title: 'Due Date';
  status      : String(20) default 'Open' @title: 'Status';  // Open/In Progress/Done/Cancelled
  priority    : String(10) default 'Medium' @title: 'Priority';  // Low/Medium/High
  source      : String(100) @title: 'Source';  // e.g. "Steering Committee 2026-05-01"
}

// ─── Progress Snapshots (weekly trend data) ───
entity ProgressSnapshots : cuid {
  workItem      : Association to WorkItems;
  snapshotDate  : Date @title: 'Date';
  deploymentPct : Decimal(5,2) @title: 'Deployment %';
  testPassRate  : Decimal(5,2) @title: 'Test Pass Rate %';
  ragStatus     : String(10) @title: 'RAG';
  testPassed    : Integer @title: 'Tests Passed';
  testTotal     : Integer @title: 'Tests Total';
}
...
// ─── Weekly Digests (AI-generated summaries — save only, no auto-email) ───
entity WeeklyDigests : cuid, managed {
  weekLabel      : String(30) @title: 'Week Label';          // e.g. "WK14 FY2026"
  application    : String(20) @title: 'Application';         // SAP / Coupa / Commercial / ALL
  digestHtml     : LargeString @title: 'Digest HTML';        // Outlook-ready HTML
  digestText     : LargeString @title: 'Digest Plain Text';  // Plain text fallback
  projectCount   : Integer default 0 @title: 'Projects Covered';
  riskCount      : Integer default 0 @title: 'Risks Identified';
  highlights     : LargeString @title: 'Key Highlights (JSON)';
  generatedBy    : String(120) @title: 'Generated By';
  aiProvider     : String(30)  @title: 'AI Provider Used';
}
