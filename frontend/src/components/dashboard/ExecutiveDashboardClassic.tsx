...
const ExecutiveDashboardClassic: React.FC = () => {
  ...
  // G��G�� Per-Application Breakdown G��G��
  ...
  return (
    <div>
        ...
        <Row align="middle" justify="space-between">
          <Col>
            ...
            <Text style={{ color: 'rgba(255,255,255,0.7)', fontSize: 14, marginTop: 4, display: 'block' }}>
              Welcome, <strong>{user?.name || 'Executive'}</strong> G�� Cross-application portfolio health
            </Text>
          </Col>
          <Col>
            <Space size="large">
      ...
      {/* G��G�� Per-Application Breakdown Cards G��G�� */}
      <Row gutter={[12, 12]}>
      ...
      <Row gutter={[16, 16]} style={{ marginTop: 16 }}>
        <Col xs={24} lg={16}>
          <Card
            ...
            size="small"
            extra={<a onClick={() => navigate('/tracker?app=all')}>View All G��</a>}
          >
            {activeProjects.length === 0 ? (
                  ...
                  children: (
                    <div className="cursor-pointer" onClick={() => navigate(`/workitem/${wi.ID}`)}>
                      ...
                      <Text type="secondary" style={{ fontSize: 11 }}>
                        {wi.goLiveDate} G�� <Tag color={wi.daysLeft <= 7 ? 'red' : wi.daysLeft <= 14 ? 'orange' : 'blue'} style={{ fontSize: 10 }}>{wi.daysLeft}d</Tag>
                      </Text>
                    </div>
                  ),
                }))}
