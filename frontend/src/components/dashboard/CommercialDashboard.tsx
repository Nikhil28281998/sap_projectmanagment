...
const CommercialDashboard: React.FC = () => {
  ...
  const goLiveCols = [
    ...
    { title: 'Health', key: 'rag', width: 70, align: 'center' as const,
      render: (_: any, r: any) => {
        const rag = getRAG(r);
        return <div className={`rag-dot ${rag === 'GREEN' ? 'rag-dot-green' : rag === 'AMBER' ? 'rag-dot-amber' : 'rag-dot-red'}`} />;
      },
    },
    ...
    return (
      <div>
        <div className="dashboard-view-toggle dashboard-toggle-classic">
          <Title level={4} className="m-0"><MedicineBoxOutlined /> Commercial Life Sciences</Title>
          <Segmented
            options={[
    ...
    <div className="analytics-dashboard">
      <div className="dashboard-view-toggle">
        <div>
          <Text className="dashboard-breadcrumb">
            SAP PM Command Center / <Text strong className="dashboard-breadcrumb-active">Commercial Initiatives Analytics</Text>
          </Text>
        </div>
        <Segmented
          options={[
      ...
      <div className="analytics-filter-bar">
        ...
        <Select placeholder="Priority" allowClear value={filterPriority} onChange={setFilterPriority}
          className="filter-select-sm" options={priorities.map((p: string) => ({ value: p, label: p }))} />
        <Select placeholder="Status" allowClear value={filterStatus} onChange={setFilterStatus}
          className="filter-select-md" options={statuses.map((s: string) => ({ value: s, label: s }))} />
      ...
      <Row gutter={16} className="mb-20">
        ...
        <Col xs={16} lg={8}>
          <div className="analytics-kpi analytics-kpi-mini">
            ...
            <Text type="secondary" className="fs-11">upcoming launches</Text>
          </div>
        </Col>
      </Row>
        ...
        <Row gutter={24}>
          <Col xs={24} lg={14}>
            <div className="chart-section-header">
              <Text strong className="fs-14">Initiatives by Phase & Health</Text>
              <br /><Text type="secondary" className="fs-12">Commercial lifecycle phases</Text>
            </div>
            <Space size={16} className="chart-section-header">
          ...
          <Col xs={24} lg={10}>
            <div className="chart-section-header">
              <Text strong className="fs-14">Initiatives by Type</Text>
              <br /><Text type="secondary" className="fs-12">Commercial initiative categories</Text>
            </div>
      ...
      {upcomingGoLives.length > 0 && (
        <div className="analytics-chart-card">
          <div className="chart-header-actions">
            <div className="chart-title mb-0">Upcoming Commercial Launches</div>
