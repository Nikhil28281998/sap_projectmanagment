...
const WorkItemDetail: React.FC = () => {
  ...
  const handleAddMilestone = async () => {
    ...
    try {
      await milestoneApi.create({
        ...
        milestoneDate: newMilestone.milestoneDate || undefined,
  ...
  const openEditMilestone = (ms: any) => {
    setEditMs({
      ...
      autoGenerated: ms.autoGenerated,
    });
  };

  const handleSaveMilestone = async () => {
    if (!editMs) return;
    if (!editMs.milestoneName.trim()) { message.warning('Milestone name is required'); return; }
    try {
      await milestoneApi.update(editMs.ID, {
        milestoneName: editMs.milestoneName.trim(),
        milestoneDate: editMs.milestoneDate || undefined,
        milestoneOrder: editMs.milestoneOrder,
        status: editMs.status,
        completedDate: editMs.status === 'Complete' ? (new Date().toISOString().split('T')[0]) : null,
        evidence: editMs.evidence || undefined,
      });
  ...
  // Build milestone timeline — sorted by milestoneOrder
  const milestones = [...(workItem.milestones || [])].sort((a: any, b: any) => (a.milestoneOrder || 0) - (b.milestoneOrder || 0));
    ...
    <div>
        ...
        <Col xs={24} lg={16}>
          <Card title="Work Item Details" size="small" style={{ marginBottom: 16 }}>
            <Descriptions bordered size="small" column={{ xs: 1, sm: 2 }}>
              ...
              {workItem.application === 'SAP' && (
                <Descriptions.Item label="AMS Ticket">
                  {editableField('amsTicket', workItem.amsTicket ?? undefined)}
                </Descriptions.Item>
              )}
            ...
            {(workItem.testTotal || 0) > 0 ? (
              <>
                ...
                <div className="wi-test-stacked-bar">
                  {(workItem.testPassed ?? 0) > 0 && (
                    <Tooltip title={`Passed: ${workItem.testPassed}`}>
                      <div className="bg-green" style={{ width: `${((workItem.testPassed ?? 0) / (workItem.testTotal || 1)) * 100}%` }} />
                    </Tooltip>
                  )}
                  {(workItem.testFailed ?? 0) > 0 && (
                    <Tooltip title={`Failed: ${workItem.testFailed}`}>
                      <div className="bg-red" style={{ width: `${((workItem.testFailed ?? 0) / (workItem.testTotal || 1)) * 100}%` }} />
                    </Tooltip>
                  )}
                  {(workItem.testBlocked ?? 0) > 0 && (
                    <Tooltip title={`Blocked: ${workItem.testBlocked}`}>
                      <div className="bg-amber" style={{ width: `${((workItem.testBlocked ?? 0) / (workItem.testTotal || 1)) * 100}%` }} />
                    </Tooltip>
                  )}
                  {(workItem.testTBD ?? 0) > 0 && (
                    <Tooltip title={`TBD: ${workItem.testTBD}`}>
                      <div className="bg-gray-light" style={{ width: `${((workItem.testTBD ?? 0) / (workItem.testTotal || 1)) * 100}%` }} />
                    </Tooltip>
                  )}
                  {(workItem.testSkipped ?? 0) > 0 && (
                    <Tooltip title={`Skipped: ${workItem.testSkipped}`}>
                      <div className="bg-gray" style={{ width: `${((workItem.testSkipped ?? 0) / (workItem.testTotal || 1)) * 100}%` }} />
                    </Tooltip>
                  )}
                </div>
              </>
              ...
              <Timeline
                items={milestones.map((m: any) => ({
                  ...
                  children: (
                    <div>
                      <Space size={4}>
                        <Text strong className={m.status === 'Complete' ? 'ms-name-done' : undefined}>{m.milestoneName}</Text>
                        {canWrite && (
                          <Tooltip title="Edit milestone">
                            <Button type="text" size="small" icon={<EditOutlined />}
                              className="ms-edit-btn" onClick={() => openEditMilestone(m)}
                              aria-label={`Edit ${m.milestoneName}`} />
                          </Tooltip>
                        )}
                        {m.autoGenerated && <Tag className="ms-auto-tag">Auto</Tag>}
                      </Space>
                      <br />
                      <Text type="secondary" className="fs-12">
                        {m.milestoneDate ? new Date(m.milestoneDate).toLocaleDateString() : 'No date set'}
                        {m.status === 'Complete' && m.completedDate && ` — Completed ${new Date(m.completedDate).toLocaleDateString()}`}
                      </Text>
                      {m.evidence && (
                        <>
                          <br />
                          <Text type="secondary" className="fs-11">{m.evidence}</Text>
                        </>
                      )}
                    </div>
                  ),
      ...
      <Modal
        ...
        footer={(_, { OkBtn, CancelBtn }) => (
          <div className="flex-between">
            <div>
              {editMs && !editMs.autoGenerated && (
                <Popconfirm title="Delete this milestone?" description="This cannot be undone."
                  onConfirm={() => handleDeleteMilestone(editMs.ID)} okText="Delete" okButtonProps={{ danger: true }}>
      ...
      >
        {editMs && (
          <Space direction="vertical" className="w-full" size="middle">
            <div>
              <Text strong className="d-block mb-4">Milestone Name *</Text>
              <Input value={editMs.milestoneName}
                onChange={(e) => setEditMs({ ...editMs, milestoneName: e.target.value })}
                placeholder="Milestone name" />
            </div>
            <Row gutter={12}>
              <Col span={12}>
                <Text strong className="d-block mb-4">Deadline Date</Text>
                <DatePicker className="w-full"
                  value={editMs.milestoneDate ? dayjs(editMs.milestoneDate) : null}
                  onChange={(d) => setEditMs({ ...editMs, milestoneDate: d ? d.format('YYYY-MM-DD') : '' })} />
              </Col>
              <Col span={12}>
                <Text strong className="d-block mb-4">Status</Text>
                <Select className="w-full" value={editMs.status}
                  onChange={(v) => setEditMs({ ...editMs, status: v })}
                  options={[
            ...
            <Row gutter={12}>
              <Col span={12}>
                <Text strong className="d-block mb-4">Order (Position)</Text>
                <InputNumber className="w-full" min={1} value={editMs.milestoneOrder}
                  onChange={(v) => setEditMs({ ...editMs, milestoneOrder: v || 1 })} />
              </Col>
              <Col span={12}>
                <Text strong className="d-block mb-4">Type</Text>
                <Input value={editMs.autoGenerated ? 'Auto-generated' : 'Manual'} disabled />
              </Col>
            </Row>
            <div>
              <Text strong className="d-block mb-4">Evidence / Notes</Text>
              <Input.TextArea rows={2} value={editMs.evidence}
                onChange={(e) => setEditMs({ ...editMs, evidence: e.target.value })}
                placeholder="e.g., Sign-off email received, UAT passed with 0 defects" />
            </div>
          </Space>
        )}
      </Modal>
